...
(function(module) {
	...
	CreateTemplateController : function(scope,resourceFactory, location,PermissionService) {
		scope.mappers = [];
		scope.formData = {};
		...
		scope.clientKeys = function() {
			 ...
			 * CKEDITOR.instances.templateeditor.setData('');
			 * scope.loanTemplateKeys = ["{{loan.accountNo}}",
			 * "{{loan.status.value}}",
			 * "{{loan.loanProductId}}",
			 * "{{loan.loanProductName}}",
			 * "{{loan.loanProductDescription}}"];
			 * scope.repaymentTemplateKeys =
	...
	mifosX.ng.application.controller('CreateTemplateController',[ 
	            '$scope', 'ResourceFactory', '$location','PermissionService',
				 mifosX.controllers.CreateTemplateController ]).run(
			function($log) { $log.info("CreateTemplateController initialized");
			});
}(mifosX.controllers || {}));
